...
UNITY_INSTANCING_BUFFER_END(Terrain)
         ...
         ) 
      {
         ...
         #if !_DEBUG_USE_TOPOLOGY && UNITY_VERSION >= 201830 && !_TERRAINBLENDABLESHADER && !_MICROMESH && !_MICRODIGGERMESH && !_MICROVERTEXMESH && defined(UNITY_INSTANCING_ENABLED) && !defined(SHADER_API_D3D11_9X)
      ...
      MicroSplatLayer Sample(Input i, half4 weights, inout Config config, float camDist, float3 worldNormalVertex)
      {
         ...
         half4 fxLevels = half4(0,0,0,0);
            ...
            half wetLevel = 0;
            #if _MICROMESH
               fxLevels = SampleFXLevels(InverseLerp(_UVMeshRange.xy, _UVMeshRange.zw, config.uv), wetLevel, burnLevel);
         ...
         GlobalSpecularTexturePerTex(samples, config, camDist, globalSlopeFilter);
         #endif
         ...
         ApplyDetailDistanceNoise(albedo.rgb, normSAO, config, camDist, i.worldPos, worldNormalVertex);
            ...
            MSBRANCHOTHER(camDist - _SplatFade.x)
            {
               ...
               half4 sfalb = MICROSPLAT_SAMPLE_TEX2D_GRAD(_Diffuse, float3(config.uv * _UVScale, _SplatFade.z), sfDX, sfDY);
               COUNTSAMPLE
               albedo.rgb = lerp(albedo.rgb, sfalb.rgb, falloff);

               #if !_NONOMALMAP
                  half4 sfnormSAO = MICROSPLAT_SAMPLE_TEX2D_GRAD(_NormalSAO, float3(config.uv * _UVScale, _SplatFade.z), sfDX, sfDY).garb;
                  COUNTSAMPLE
         ...
         GeoTexture(albedo.rgb, normSAO, i.worldPos, worldHeight, config, worldNormalVertex, upVector);
            ...
            ApplyPlanet(i, albedo.rgb, normSAO, config, camDist, i.worldPos, worldNormalVertex);
         #endif
         ...
         GlobalSpecularTexture(specular.rgb, config, camDist, globalSlopeFilter);
         #endif
         ...
         snowCover = DoSnow(o, config.uv, WorldNormalVector(i, o.Normal), worldNormalVertex, i.worldPos, pud, porosity, camDist, 
            config, weights, SSSTint, SSSThickness);
         #endif
...
ENDCG
   ...
   Dependency "BaseMapGenShader" = "/Example_Base-1146652834"
   Dependency "BaseMapShader" = "Hidden/MicroSplat/Example_Base-1146652834"
